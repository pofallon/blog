...
In the last two weeks, Anthropic shipped two features that change how developers interact with AI coding agents. Remote Control lets you continue a local Claude Code session from your phone. Channels let you push messages from Telegram or Discord into a running session so your agent can react while you're away from the keyboard.

These are genuinely exciting capabilities. But they share an assumption that nobody seems to be questioning: the agent is running _somewhere_ reliable.

For most of us, it isn't.

## The Two Options (and Why Neither Works)

Today, if you want to run an AI coding agent, you have two choices.
...
**A cloud black box.** Claude Code on the web runs in an Anthropic-managed cloud sandbox. It's persistent and you can walk away. But you can't SSH in. You can't customize the environment. You can't inspect what's happening between the commits. You get the output but not the transparency.
...
When Anthropic's Channels send a Telegram message to your agent, that message arrives in an environment you provisioned, configured, and can inspect. When Remote Control lets you steer from your phone, you're steering a session running in a place you understand.
