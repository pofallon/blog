...
### User Story 2 - PlaylistEmbed parity (Priority: P2)

Readers need the `PlaylistEmbed` component inside MDX posts that mirrors the current playlist-style embed (track list, play controls, metadata) so existing editorial experiences are preserved.
...
**Independent Test**: Publish a post that swaps the legacy playlist embed for the `PlaylistEmbed` component and confirm the controls, track sequencing, and analytics hooks behave exactly as they do today.

**Acceptance Scenarios**:

1. **Given** a post uses the `PlaylistEmbed` component with valid data, **When** a reader interacts with play/pause and track selection, **Then** the component mirrors current playlist behavior and visual styling.  
2. **Given** the `PlaylistEmbed` component requires external media metadata, **When** that data is temporarily unavailable, **Then** the component displays a friendly fallback state without breaking the rest of the article.  
3. **Given** optional API keys unlock enhanced `PlaylistEmbed` features, **When** the keys are not provided, **Then** the component continues to render core controls and clearly omits the premium features.
...
- Referencing the same registered component multiple times in a single post must not create duplicate global IDs or conflicting state; each instance maintains isolated state with no cross-instance communication.  
...
- **FR-002**: The MDX compilation pipeline MUST resolve component references only if they exist in the registry, emit non-blocking validation warnings for unknown names at build time, and render a graceful fallback placeholder at runtime for unregistered references.  
...
- **FR-004**: The registry MUST include the `PlaylistEmbed` interactive component whose behavior, content fields, and analytics triggers match the existing playlist-style embed.  
...
- **FR-006**: The system MUST surface actionable validation output (preview UI and build logs) whenever component props are missing, malformed, or use deprecated fields; all validation and credential errors MUST be written to existing application logs with structured tags (e.g., `component:registry`, `severity:warning`) for filtering.  
...
- **Component Registry Entry**: Describes an approved interactive element (e.g., `PlaylistEmbed`), listing canonical name, version, description, required/optional props, and dependencies such as APIs or data feeds.  
...
- Enabling the `PlaylistEmbed` interactive component through that registry with full parity to the current embed.  
...
- Governance workflow for new component requests (FR-010) - deferred to post-MVP; initial components are curated by maintainers.
...
2. **Performance Impact**: Overuse of interactive components could slow page loads if fallbacks are not optimized. Components must not increase Largest Contentful Paint (LCP) by more than 500ms; exceeding this threshold blocks deployment.  
...
- Q: What is the acceptable performance impact threshold for registry components on page load? → A: Components must not increase LCP by more than 500ms.
- Q: How should multiple instances of the same component on one page handle state? → A: Each instance maintains isolated state, no cross-instance communication.
- Q: What behavior should unregistered component references trigger? → A: Warnings at build, graceful fallback at runtime.
- Q: Where should validation errors and credential issues be logged? → A: Existing application logs with structured tags for filtering.
- Q: What should the canonical name for the playlist component be? → A: PlaylistEmbed.
...
- **SC-003**: The `PlaylistEmbed` component achieves feature parity verification with zero regressions identified in launch QA and maintains the current completion rate benchmark within ±5%.  
